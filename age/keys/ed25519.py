...
from cryptography.hazmat.primitives.asymmetric.ed25519 import (
    ...
    Ed25519PublicKey as Ed25519PublicKey_,
)
...
class Ed25519PrivateKey(DecryptionKey):
    ...
    def from_pem(cls, pem_data: bytes, password: bytes = None):
        try:
            ...
            private_key = load_openssh_private_key(
                pem_data, passphrase=password
            )
    ...
    def to_age_private_key(self) -> AgePrivateKey:
        ...
        ed25519_sk = self._key.private_bytes(
            Encoding.Raw, PrivateFormat.Raw, NoEncryption()
        )
        curve25519_sk = crypto_sign_ed25519_sk_to_curve25519(
...
class Ed25519PublicKey(EncryptionKey):
    ...
    def from_ssh_public_key(cls, ssh_public_key_data: bytes):
        ...
        public_key = load_ssh_public_key(
            data=ssh_public_key_data, backend=default_backend()
        )
        assert isinstance(public_key, Ed25519PublicKey_)
        return cls(public_key)

    def binary_encoding(self) -> bytes:
        serialized = self._key.public_bytes(
    ...
    def fingerprint_line(
        self,
        algorithm: str = "MD5",
        comment: str = OPENSSH_DEFAULT_KEY_COMMENT,
    ) -> str:
        return " ".join(
            [
                str(ED25519_KEY_SIZE),
                self.fingerprint(algorithm=algorithm),
                comment,
                "(ED25519)",
            ]
        )
