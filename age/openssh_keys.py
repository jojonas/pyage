...
def _deserialize_dsa_private_key(stream: typing.BinaryIO) -> dsa.DSAPrivateKey:
    ...
    public_numbers = dsa.DSAPublicNumbers(
        y=dsa_pub_key, parameter_numbers=parameter_numbers
    )
    private_numbers = dsa.DSAPrivateNumbers(
        x=dsa_priv_key, public_numbers=public_numbers
    )
    return private_numbers.private_key(backend=default_backend())


def _deserialize_ecdsa_private_key(
...
def _deserialize_ed25519_private_key(
    stream: typing.BinaryIO
) -> ed25519.Ed25519PrivateKey:
...
def _decrypt_key(
    ...
    if kdfname != b"bcrypt":
        raise InvalidKeyFile(
            f"Unsupported private key encryption KDF {kdfname}"
        )
    ...
    if ciphername not in suites:
        raise NotImplementedError(
            f"Unsupported private key encryption cipher {ciphername}"
        )
...
def load_openssh_private_key(
    openssh_data: bytes, passphrase: bytes = None
) -> AnyPrivateKey:
    ...
    base64_encoded_part = openssh_data[
        len(OPENSSH_HEADER) : -len(OPENSSH_FOOTER)
    ].replace(b"\n", b"")
    ...
    decrypted = _decrypt_key(
        encrypted, passphrase, ciphername, kdfname, kdf_metadata
    )
    ...
    if key_type == b"ssh-rsa":
        ...
        raise NotImplementedError(
            f"SSH key type {key_type} not implemented yet."
        )
