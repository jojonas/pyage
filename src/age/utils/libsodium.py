...
def crypto_box_open_easy_afternm(ctxt, nonce, k):
    ...
    if len(nonce) != crypto_box_NONCEBYTES:
        raise ValueError("Invalid nonce")
    msg = ctypes.create_string_buffer(len(ctxt) - crypto_box_MACBYTES)
    ret = nacl.crypto_box_open_easy_afternm(msg, ctxt, ctypes.c_ulonglong(len(ctxt)), nonce, k)
    if ret:
        raise CryptError("unable to decrypt message")
    return msg.raw
...
def crypto_onetimeauth_verify(token, message, key):
    ...
    ret = nacl.crypto_onetimeauth_verify(token, message, ctypes.c_ulonglong(len(message)), key)
    if ret:
...
def crypto_box_seed_keypair2(seed):
    """
